...
async function processWithProductionAPI(options) {
    ...
    for (let i = 0; i < totalFiles; i++) {
        ...
        try {
            // Generate unique jobId for this file's variants
            const jobId = Date.now().toString(36) + Math.random().toString(36).substr(2);
            const outputVariants = [];
            const storedFileName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, '_');

            // Optionally upload the original image
            const originalData = await getPresignedUrl(file, jobId, 'original');
            await uploadToS3(file, originalData);

            // Resize and Upload variants
            if (options.resize1080p) {
                const blob1080 = await resizeImageClientSide(file, 1920, 1080);
                const data1080 = await getPresignedUrl(blob1080, jobId, '1080p', storedFileName);
                await uploadToS3(blob1080, data1080);
                outputVariants.push(buildVariantFromUploadData(data1080, '1080p'));
            }
            if (options.resize720p) {
                const blob720 = await resizeImageClientSide(file, 1280, 720);
                const data720 = await getPresignedUrl(blob720, jobId, '720p', storedFileName);
                await uploadToS3(blob720, data720);
                outputVariants.push(buildVariantFromUploadData(data720, '720p'));
            }
            if (options.resize480p) {
                const blob480 = await resizeImageClientSide(file, 854, 480);
                const data480 = await getPresignedUrl(blob480, jobId, '480p', storedFileName);
                await uploadToS3(blob480, data480);
                outputVariants.push(buildVariantFromUploadData(data480, '480p'));
            }
            ...
            showNotification(`Processed & Uploaded: ${file.name}`, 'success');
        } catch (err) {
...
async function getPresignedUrl(fileOrBlob, jobId, folder, overrideName = null) {
    const fileName = overrideName || (fileOrBlob.name || 'image.jpg');
...
// Build expected S3 URLs from the direct upload data
function buildVariantFromUploadData(uploadData, label) {
    const baseUrl = CONFIG.OUTPUT_BASE_URL.replace(/\/+$/, '');
    const key = uploadData.fileUrl || uploadData.fields.key;
    const url = `${baseUrl}/${encodeURI(key)}`;
    return { label, key, url };
}

// Client-Side Resizing using HTML5 Canvas
function resizeImageClientSide(file, maxWidth, maxHeight) {
